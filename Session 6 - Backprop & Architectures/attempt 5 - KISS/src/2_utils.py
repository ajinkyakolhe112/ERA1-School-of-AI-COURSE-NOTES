...
def train(train_loader, model, error_func, optimizer, epoch_no, device=None):
	...
	for b_id, (images, labels) in enumerate(pbar):
		...
		train_accuracy_batch = 100.0 * correct_preds_batch / BATCH_SIZE
		pbar.set_description("Train: Epoch %d, Batch= %d, Batch Error Value = %f, Batch Pred Acc = %f" % (epoch+1,b_id+1, train_loss_batch.item(), train_accuracy_batch))
		correct_preds_total = correct_preds_total + correct_preds_batch
		train_loss_total = train_loss_batch.item()
		if (b_id + 1) % 100 == 0:
			print("Train: Epoch %d, Batch %d, Error Value = %f, Pred_Acc = %f" % (epoch+1,b_id+1, train_loss_batch.item(), train_accuracy_batch))
