...
class S9_Baseline(nn.Module):
	def __init__(m):
		...
		m.block1 = nn.ModuleDict({
			"conv1": nn.Conv2d(1,32,(7,7)),
			"relu": RELU()

		})
		m.block2 = nn.ModuleDict({
			"conv1": nn.Conv2d(32,32,(8,8)),
			"relu": RELU(),
		})
	...
	def forward(m,input_batch):
		...
		output_classes_neurons = m.fc1(b4_output)
		
		output_probs = nn.functional.softmax(output_classes_neurons,dim=1)
		output_log = m.log_softmax(output_classes_neurons, dim=1)
