...
Original file is located at
    https://colab.research.google.com/drive/1gbN0fsB236mQEu3uJIpXQWx55YuRsLKV
"""
...
import torch # import torch module. It contains Tensors, Random Sampling methods group, Math Operations & torch compile . (Imp modules for documentation)
# to check what each module is importing, execute
# dir(torch)
# vars(torch)

#
import torch.nn as nn # nn basic building block for NN. Layers, Activations, Loss Functions. Each is extendible. 
# Base: nn.Containers module. Imp once are (nn.Module, nn.Sequential), nn.ParameterDict. Extra forward & backward pre & post hooks.  
# Details of nn.Module: essential methods to override init & forward. Module.training<Status> represents whether this module is in training or evaluation mode.
import torch.nn.functional as F # module for grouping of all functions in pytorch. Internally in Conv2d layer, respective function from here is called

#
import torch.optim as optim # Package implementing various optimization algorithms. SGD, ADAM variations. 
...
#
from torchvision import datasets, transforms # Seperate library, part of Pytorch. Doesn't mess up generic torch module. And specialized vision related are organized here
# 1. popular datasets, 2. model architectures, and 3. image transformations for computer vision.

# Important for checking parameters & output size. But has been superseded by torchinfo package. That one has verbose customizable output compared to this one
#!pip install torchsummary
from torchsummary import summary

use_cuda = torch.cuda.is_available()       
device = torch.device("cuda" if use_cuda else "cpu") # cuda default accelerator for DL. Also have tpu which is accelerated for pytorch with XLA. 
device
...
batch_size = 128  # each batch leads to one update of parameters. All 128 images would be run parallely, hence speedup achieved.
# I don't know what is the upper limit of the batch_size. 

train_loader = torch.utils.data.DataLoader( # At the heart of PyTorch data loading utility is the torch.utils.data.DataLoader class. It represents a Python iterable over a dataset
    datasets.MNIST('../data', train=True, download=True, # torchvision.datasets for all vision datasets. Has download from internet option. 
                    transform=transforms.Compose([   # torchvision.datasets does tranforms of Data .. and torch.utils.data.DataLoader does batching... 
                        transforms.ToTensor(),
...
class FirstDNN(nn.Module):  # nn.Module either model or layer or block of layers
  def __init__(self):
  ...
  def forward(self, x): # x is the input to module or layer. x is Example or Data to be transformed by layer
    ...
    # This has been sequential forward pass. If wanted to branch out, can do those transformations as well. And accordingly, error will be propogated automatically by autograd
